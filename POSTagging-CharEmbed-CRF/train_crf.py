...
from CRFTaggerModel import POSTagger
...
def calc_acc(pred, target, mask):
    pred = torch.masked_select(pred, mask)
    target = torch.masked_select(target, mask)
    return torch.eq(pred, target).cpu().sum().item()
...
def evaluate(test_data, tagger, vocab, char_vocab, config):
    ...
    for batch_data in get_batch(test_data, config.batch_size):
        xb, xch, yb, mask, seq_lens = batch_variable_mask_easy(batch_data, vocab, char_vocab)
        if config.use_cuda:
            xb = xb.cuda()
            xch = xch.cuda()
            yb = yb.cuda()
...
def train(train_data, test_data, dev_data, vocab, config, wd_embed_weights, char_embed_weights):
    ...
    for ep in range(config.epochs):
        ...
        for batch_data in get_batch(train_data, config.batch_size):
            # 1、准备数据
            xb, xch, yb, mask, seq_lens = batch_variable_mask_easy(batch_data, vocab, char_vocab)  # 处理batch_data
